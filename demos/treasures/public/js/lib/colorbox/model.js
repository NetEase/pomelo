...
var viewClipModel = (function()
                     {
                       ...
                       return function(model, vr)
                       {
                         ...
                         if (!offscreen_buffer)
                         {
                           offscreen_buffer = pjs.createGraphics(pjs.width, pjs.height, pjs.P2D);
                         }

                         offscreen_buffer.beginDraw();

                         offscreen_buffer.background(0, 0, 0, 0);
                         offscreen_buffer.pushMatrix();
                         offscreen_buffer.translate(-model.x, -model.y);
                         
                         vr.draw(model.model, offscreen_buffer);
