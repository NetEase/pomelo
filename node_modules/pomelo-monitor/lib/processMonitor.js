...
function format(server,data,cb){
	...
	for(var i=0;i<outArray.length;i++){
		...
		exec('pidstat -p '+pid,function(error,output){
			var pidValueArray=[];
			 var pidArray=output.toString().replace(/^\s+|\s+$/g,"").split(/\s+/);
			 for(var i=0;i<pidArray.length;i++){
		     if((!isNaN(pidArray[i]))){
			 pidValueArray.push(pidArray[i]);
		     }
		 }
		 ps.usr=pidValueArray[1];
		 ps.sys=pidValueArray[2];
		 ps.gue=pidValueArray[3];
		 cb(ps);
		});
...
// >>>>>>> c8b2a718c3cbbb7524b310bd528c432681b14309
