 ...
 * @param dir {String} handler codes root dir in abosulte path
 * @param scope {String} user or sys, default is user
 */
var genProxy = function(app, name, dir, scope) {
   ...
   *
   * @param namespace {String} namespace for the proxy, eg: sys.connector.helloService
   * @param mod {Object} the original module instance that would be delegated
	...
	function proxyCallback(namespace, mod) {
        ...
        opts.type = attach.type;
				...
				app.get('mailRouter').route(opts, function(err, serverId) {
					if(!!err) {
						logger.error('fail to route type:' + attach.type + ', uid:' + opts.uid + ', err:' + err.stack);
					}
  ...
  var proxies = {};
  Loader.loadPath({path: dir, recurse: false, log:false, namespace: scope, rootObj: proxies, callback: proxyCallback});
  return proxies;
};  //end of genProxy
