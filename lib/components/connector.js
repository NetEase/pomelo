...
Component.prototype.setPublicKey = function(key){
  this.key = key;
};
...
var bindEvents = function(self, socket) {
  ...
  socket.on('message', function(msg) {
    ...
    // use rsa crypto
    if(self.useCrypto) {
      var verified = verifyMessage(self, dmsg);
      if(!verified) {
...
var verifyMessage = function (self, msg) {
  ...
  publicKey.n = new rsa.BigInteger(self.key.rsa_n, 16);
  publicKey.e = self.key.rsa_e;
  ...
  return publicKey.verifyString(JSON.stringify(msg.body), sig);
};