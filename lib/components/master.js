...
var debug = function (app, server, port) {
    var cmd = 'cd ' + process.cwd() + ' && node --debug=' + port + '  ' + app.main + '  ' + app.env + '  ' + server.serverType + ' ' + server.id;
    if (server.host == '127.0.0.1' || server.host == 'localhost') {
        cmd = 'cd ' + process.cwd() + ' && node --debug=' + port + '  ' + app.main + '  ' + app.env + '  ' + server.serverType + ' ' + server.id;
        starter.run(cmd);
    } else {
        starter.sshrun(cmd, server.host);
    }
