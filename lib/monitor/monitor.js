...
Monitor.prototype.start = function(cb) {
  if(this.app.enabled('masterHA') && (this.app.getServerType()!== 'master')){
    logger.warn('bind masterupdate for %j', this.app.serverId);
...
Monitor.prototype.reconnect = function(){
  logger.error('server %j reconnect to master!', this.app.serverId);
  var closure = this;
  this.zookeeper.getData(function(err, masterInfo){
    if(err || !masterInfo){
      logger.error('get masterInfo error!');
      return;
    }

    masterInfo = JSON.parse(masterInfo);
    logger.warn('new master info : %j', masterInfo);
