...
// MonitorAgent gets registered as a Node on the LogServer.
...
MonitorAgent.prototype = {
	...
	connect : function() {
		...
		function _log_file_helper(message, func) {
			var log_file = monitorAgent.log_files[message.log_file];
			if (log_file == null) {
				return;
			}
		...
		monitorAgent.socket.on('enable_log', function(message) {
			_log_file_helper(message, function(log_file) {
				monitorAgent._log.info("Enabling log file: " + log_file.label);
				log_file.enable();
		...
		monitorAgent.socket.on('disable_log', function(message) {
			_log_file_helper(message, function(log_file) {
				monitorAgent._log.info("Disabling log file: " + log_file.label);
				log_file.disable();
			});
		});
			...
			if (type === 'CPU') {
				...
				snapshot.serialize({
					onData : function(chunk, size) {
						chunk = chunk + '';
						monitorAgent.socket.emit('profiler', {
	...
	run : function() {
		...
		__(monitorAgent.log_files).each(function(log_file, label) {
			log_file.watch();
			// monitorAgent._log.info("Watching: " + log_file.path + " (" + label +
			// ")");
		});
		...
		// setInterval(function() {
		// monitorAgent._log.info("Watching " + __(monitorAgent.log_files).size()
		// + " log files, " + " sent " + monitorAgent.messages_sent
		// + " log messages.");
		// }, monitorAgent._conf.status_frequency);
	},
