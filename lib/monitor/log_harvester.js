...
//  		console.error('server node get message from master  ' + harvester.socket + ' ' + JSON.stringify(message));
    	if (message.method==='getSystem') {
    		var sysMsg=monitor.getSysInfo(function(message){
//    			console.error("sysMsg@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@:"+JSON.stringify(message));
    			return message;
    		});
    		harvester.socket.emit('clientmessage',{method:message.method,body:sysMsg});
    	}	else {
