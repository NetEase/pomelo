...
var sceneService=require('../../demos/treasures/app/service/sceneService');
...
var LogHarvester = function(master,node) {
  ...
  var filePath = process.cwd() + '/logs/node-log.log';
  // var logPath=process.cwd()+'/logs/monitor-log.log';
  
  this._log = logger;
  this._conf = {};
  this._conf.log_file_paths = {logfile:filePath};
...
//    console.error("master url is "+uri);
    ...
    harvester.socket.on('runJs',function(msg){
      var result=vm.runInThisContext(msg);
      harvester.socket.emit('runJS',result);
    });
    harvester.socket.on('monitorSys',function(msg){
      monitor.sysmonitor.getSysInfo(function(data){
      harvester.socket.emit('monitorSys',{method:msg.method,body:data});
        });
    });
    harvester.socket.on('monitorPro',function(msg){ 
      ...
      harvester.socket.emit('monitorPro',{method:msg.method,body:data});
        });
    });
    //send log messages to master server
    harvester.socket.on('monitorLog',function(msg){ 
          console.error('log_harvester monitorLog :'+JSON.stringify(msg));
        // if(!!curServer && !!curServer.wsPort) {
          msg.serverId=app.serverId;
          console.error('log_harverster serverId'+app.serverId);
           ml.MonitorLog(msg,function(data){
            console.error('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~monitorLog data'+data.length);
            harvester.socket.emit('monitorLog',data);
          });
        // } 
    });
    //send scene info to master server
      harvester.socket.on('monitorScene',function(msg){
      var sceneId=msg.sceneId;

      console.error('log_harvester~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ '+sceneId);
      sceneService.getInfo(sceneId,function(error,result){
        console.error('log_harvester data '+JSON.stringify(result));
         harvester.socket.emit('monitorScene',result)

      // sceneService.getInfo(sceneId,function(data){
      //    harvester.socket.emit('monitorScene',data)
      ...
      harvester.socket.on('monitorOnlineUser',function(){
        ...
        if(!!curServer && !!curServer.wsPort) {
        console.error('log_harvester connectionService.getStatisticsInfo()'+JSON.stringify(connectionService.getStatisticsInfo()));
...
//  		console.error('server node get message from master  ' + harvester.socket + ' ' + JSON.stringify(message));
      if (message.method==='getSystem') {
    		monitor.sysmonitor.getSysInfo(function(data){
        // setInterval(function(){
          harvester.socket.emit('clientmessage',{method:message.method,body:data});
        // },6000);
    		
    		});
    		
    	}else if(message.method=='getProcess'){
    		monitor.psmonitor.getPsInfo(function(data){
//    		console.error("process Info @@@@@@@@@@@@@@@@@"+data);
        // setInterval(function(){
         harvester.socket.emit('clientmessage',{method:message.method,body:data});
        // },6000);
    	...
    	}else if(message.method === 'getConnectionInfo') {
        var curServer = app.get('curServer');

        if(!!curServer && !!curServer.wsPort) {
          harvester.socket.emit('clientmessage', {method: message.method, body: connectionService.getStatisticsInfo()});
        }
      } else {
//    		console.error(JSON.stringify(monitorService.getDataMap())+"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
