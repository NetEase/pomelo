...
pro.add = function(uid, sid) {
	return add(uid, sid, this.groups, this.defaultGroup);
};
...
pro.leave = function(uid, sid) {
	...
	var idx = _.indexOf(this.groups, uid);
	if(idx >= 0) {
...
var regroup = function(groups, defGroup, cb) {
	...
	proxy.user.status.statusService.queryStatusBatch(defGroup, function(err, res, miss) {
		if(!!err) {
