...
function run(config){
  ...
  var env = app.get('env');
    ...
    console.error('!!!!!!!!!!!!!!!!!!! triggers : ' + triggers + ' serverType :' + serverType + ' config' + JSON.stringify(config));
  if(env === 'development'){
    for(var key in config){
      var triggers = config[key];
      for(var key in triggers){
        var trigger = triggers[key];
        schedule.scheduleJob(trigger[0], mailJob, {str:trigger[1], params:trigger[2]});
      }
    }
  }else if(env === 'production' || env === 'localpro'){
    var serverType = app.get('serverType');
    var triggers = config[serverType];
    for(var key in triggers){
      var trigger = triggers[key];
      schedule.scheduleJob(trigger[0], mailJob, {str:trigger[1], params:trigger[2]});
    }
