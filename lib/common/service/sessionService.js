...
SessionService.prototype.bind = function(sid, uid, cb) {
  ...
  // statusService check uid with multiple sessions
  if(!!pomelo.app.get('statusService') && !!sessions) {
    process.nextTick(function() {
      cb(new Error('statusService is opened, session has already bind with uid: ' + uid));
    });
    return;
  }
