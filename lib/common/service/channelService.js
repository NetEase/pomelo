...
ChannelService.prototype.broadcast = function(stype, route, msg, opts, cb) {
  ...
  var genCB = function(id) {
    return function(err, fails) {
      if(err) {
        perror.error('[broadcast] fail to push message to %j, err:' + err.stack, id);
        latch.done();
        return;
      }
      successFlag = true;
...
var add = function(uid, sid, groups) {
  if(!sid) {
    pwarn.warn('ignore uid %j for sid not specified.', uid);
    return false;
  }
...
var sendMessageByGroup = function(channelService, route, msg, groups, cb) {
  ...
  var rpcCB = function(err, fails) {
    if(err) {
      perror.error('[pushMessage] fail to dispatch msg, err:' + err.stack);
      latch.done();
      return;
    }
    if(fails) {
