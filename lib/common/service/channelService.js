...
/**
 * Broadcast message to all the clients that connected to the server indicated by id.
 *
 ...
 * @memberOf ChannelService
 */
ChannelService.prototype.broadcastByServerId = function(sid, route, msg, opts, cb) {
  var app = this.app;
  var namespace = 'sys';
  var service = 'channelRemote';
  var method = 'broadcast';
  ...
  var latch = countDownLatch.createCountDownLatch(1, function() {
    if(!successFlag) {
      utils.invokeCallback(cb, new Error('broadcast fails'));
      return;
    }
  ...
  app.rpcInvoke(sid, {namespace: namespace, service: service, method: method, args: [route, msg, opts]}, genCB());
};
