...
Remote.prototype.pushMessage = function(route, msg, uids, cb) {
  ...
  var fails = [], sids = [], session;
  for(var i=0, l=uids.length; i<l; i++) {
    session = sessionService.getByUid(uids[i]);
    if(!session) {
      fails.push(uids[i]);
    } else {
  ...
  this.app.components.__scheduler__.schedule(route, msg, sids);
  cb(null, fails);
};
...
Remote.prototype.broadcast = function(route, msg, opts, cb) {
  ...
  this.app.components.__scheduler__.schedule(route, msg, null,
      opts);
