...
/**
 * close handler wrapper for backend server
 * @param app
 * @param sid
 * @param handler
 */
var closed = function(server, handler, session) {
  if(!session || !server || !handler){
      return;
  }
  server.globalHandle({'route':handler}, session, function(err, resp, opts) {});
};

/**
 * Bind the onClose Handler from backend server to frontend session.
 * @param sid
 * @param handler
 * @param cb
 */
Remote.prototype.onClose = function(sid, handler, cb) {
  var session = this.app.get('sessionService').get(sid);
  if(!session) {
    utils.invokeCallback(cb, new Error('session does not exist, sid: ' + sid));
    return;
  }
  session.on("closed", closed.bind(null, this.app.components.__server__, handler));
