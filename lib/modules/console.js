...
var add = function(app, msg, cb) {
  if(checkCluster(msg) > 0) {
    var serverMap = {};
    var fails = [];
    var successFlag;
    var server = parseArgs(msg, ClusterInfo, cb);
    utils.loadCluster(server, serverMap);
...
function checkCluster(msg) {
  ...
  var args = msg.args;
  for(var i=0; i < args.length; i++) {
    if(utils.startsWith(args[i],Constants.Reserved.START_CLUSTER) || utils.startsWith(args[i], Constants.Reserved.CLUSTER_COUNT)) {
      flag ++;
    }
  }
  return flag;
