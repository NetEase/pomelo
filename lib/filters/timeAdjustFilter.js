...
filter.handle = function(msg, session, next){
  if(!session || !session.key) {
	...
	if (Math.abs(sessionTime.diff - diff)>1000){ // diff is illegal, maybe client is cheating, illegal
	...
	// over 30 minutes elapsed , readjust sessionTime
	if (serverTimestamp - sessionTime.serverTimestamp > 30*60*1000){
		sessionTime = {clientTimestamp: clientTimestamp, serverTimestamp: serverTimestamp, diff: diff};
