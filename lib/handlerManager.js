...
manager.handle = function(msg, session, fn){
	...
	if(app.serverType === 'all' || app.serverType === type) {
	  ...
	  handler.method.call(handler.service, msg, session, function(){
	  	var endTime = Date.now();
	  	var invokeTime = endTime - startTime;
	  	fn();
	  });   // use 代理
	  return;
	}
	...
	try {
		proxy.sys[type].msgService.forwardMessage({uid:session.uid, areaId:session.areaId}, msg, session.exportSession(), function(err, resp) {
			if(!!err) {
