...
server.afterStart = function() {
  ...
  MailBox.createStation({servers: app.servers}, function(err, station) {
  	...
  	mailBox = station;
  	mailBox.addPreFilter(rpcPreFilter);
  	mailBox.addAfterFilter(rpcAfterFilter);
	...
	wsocket.sockets.on('connection', function (socket) {
		...
		socket.on('message', function(msg) {
			...
			app.handle(msg, session.cloneSession(), function(err) {
				...
				msg.handleTime = Date.now()-startTime();
				//logMsg(JSON.stringify(msg));
...
var rpcPreFilter = function(serverId, msg, opts, next) {
	opts = opts||{};
	opts.__start_time__ = Date.now();
	next(serverId, msg, opts);
};

var rpcAfterFilter = function(serverId, msg, opts, next) {
	if(!!opts && !!opts.__start_time__)
		console.log(msg.method + ' use time:' + (Date.now() - opts.__start_time__));
	next(serverId, msg, opts);
};
