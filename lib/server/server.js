...
var handleError = function(server, err, msg, session, resp, cb) {
  ...
  if(!handler) {
    perror.error('no default error handler to resolve unknown exception. ' + err.stack);
    cb(err, resp);
  } else {
...
var doForward = function(app, msg, session, routeRecord, cb) {
  ...
  try {
    app.sysrpc[routeRecord.serverType].msgRemote.forwardMessage(
      ...
      function(err, resp) {
        if(err) {
          perror.error('fail to process remote message:' + err.stack);
        }
        finished = true;
        cb(err, resp);
      }
    ...
    if(!finished) {
      perror.error('fail to forward message:' + err.stack);
      cb(err);
    }
  }
};
