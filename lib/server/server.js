...
var loadCrons = function(server, app) {
  ...
  var p = path.join(app.getBase(), Constants.FILEPATH.CRON) || path.join(app.getBase(), Constants.FILEPATH.CONFIG_DIR, env, path.basename(Constants.FILEPATH.CRON));
  if(!fs.existsSync(p)) {
    return;
  }
  app.loadConfig(Constants.RESERVED.CRONS, Constants.FILEPATH.CRON);
