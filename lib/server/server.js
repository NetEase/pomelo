...
pro.handle = function(msg, session, cb) {
  ...
  if(!routeRecord) {
	...
	if (this.app.enabled('handleSerial')) {
		taskManager.addTask(session.id, function(task){
			if(this.app.getServerType() !== routeRecord.serverType) {
				doForward(this.app, msg, session, routeRecord, function(){
					cb.apply(cb, arguments);
					task.done();
				});
			} else {
				doHandle(this, msg, session, routeRecord, function(){
					cb.apply(cb, arguments);
					task.done();
				});
			}
			...
			function(){
				...
				logger.warn('handler taskManager timeout', session, msg);
			});
	} else {
		if(this.app.getServerType() !== routeRecord.serverType) {
