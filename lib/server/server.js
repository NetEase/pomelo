...
pro.addActivities = function(activities) {
  ...
  for(var i=0, l=activities.length; i<l; i++) {
    ...
    if(!containActivity(activity.id, this.activities)) {
...
var scheduleActivities = function(server, activities) {
  ...
  for(var i = 0; i<activities.length; i++) {
    ...
    if(!handler) {
      logger.warn('could not find activity: %j', activityInfo);
      continue;
    }
    if(typeof handler[job] !== 'function') {
      logger.warn('could not find activity job: %j, %s', activityInfo, job);
      continue;
    }
    var id = schedule.scheduleJob(time, handler[job]);
