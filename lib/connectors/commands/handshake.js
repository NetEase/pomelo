...
var response = function(socket, heartbeat, resp) {
	console.error('heartbeat' + heartbeat);
  var res = {
    sys: {
      heartbeat: heartbeat
    }
  ...
  socket.handshakeResponse(protocol.encode(PKG_HANDSHAKE, new Buffer(JSON.stringify(res))));
};
...
var processError = function(socket, code) {
  ...
  socket.sendForce(protocol.encode(PKG_HANDSHAKE, new Buffer(JSON.stringify(res))));
};
