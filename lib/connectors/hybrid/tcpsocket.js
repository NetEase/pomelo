...
var readHead = function(socket, data, offset) {
  ...
  if(socket.headOffset === socket.headSize) {
    ...
    logger.error('close the connection with invalid head message, the remote ip is %s && port is %s', socket._socket.remoteAddress, socket._socket.remotePort);
    socket.close();
  }
...
var readBody = function(socket, data, offset) {
  ...
  if(socket.packageOffset === socket.packageSize) {
    ...
    logger.error('close the connection with invalid body message, the remote ip is %s && port is %s', socket._socket.remoteAddress, socket._socket.remotePort);
    socket.close();
  }
