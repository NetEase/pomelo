...
Adaptor.prototype.onPublish = function(client, packet) {
  ...
  if (!route) {
    throw new PmlError('unspecified publish route.');
  }
...
Adaptor.prototype.onSubscribe = function(client, packet) {
  ...
  if (!route) {
    throw new PmlError('unspecified subscribe route.');
  }
 ...
 * packet.id is pass from client in packet.messageId and
 * record in Pomelo context and attached to the subscribe response packet.
 *
