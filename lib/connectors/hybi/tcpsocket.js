...
var ondata = function(socket, chunk) {
  ...
  while(offset < end) {
    console.error('while %j/%j', offset, end);
...
var readHead = function(socket, data, offset) {
  console.error('package head: %j/%j', socket.headOffset, socket.headSize);
...
var readBody = function(socket, data, offset) {
  console.error('package pkg: %j/%j', socket.packageOffset, socket.packageSize);
