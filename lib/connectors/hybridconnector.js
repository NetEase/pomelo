...
Connector.prototype.start = function(cb) {
  ...
  for(var i=0; i<this.ports.length; i++) {
    ...
    var switcher = new Switcher(tcpServer, self.timeout);

    this.tcpServers.push(tcpServer);
    this.switchers.push(switcher);

    switcher.on('connection', function(socket) {
    ...
    if(this.distinctHost) {
      tcpServer.listen(this.ports[i], this.host);
    } else {
      tcpServer.listen(this.ports[i]);
    }
  }
  process.nextTick(cb);
};
...
Connector.prototype.stop = function(force, cb) {
  for(var i=0; i<this.ports.length; i++) {
    this.switchers[i].close();
    this.tcpServers[i].close();
  }
  process.nextTick(cb);
};
