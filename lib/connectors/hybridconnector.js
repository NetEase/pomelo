...
Connector.decode = Connector.prototype.decode = function(msg) {
  ...
  var decodeIO_protobuf = app.components.__decodeIO__protobuf__;
  ...
  } else if(!!decodeIO_protobuf && !!decodeIO_protobuf.check(msg.route)) {
    msg.body = decodeIO_protobuf.build(msg.route).decode(msg.body);
  } else {
    msg.body = JSON.parse(msg.body.toString('utf8'));
  }
...
var encodeBody = function(route, msgBody) {
  ...
  } else if(!!decodeIO_protobuf && !!decodeIO_protobuf.check(route)) {
     msgBody = decodeIO_protobuf.encode(decodeIO_protobuf.build(route), msgBody);
  } else {
