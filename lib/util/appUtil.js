...
var loadServers = function(app) {
  app.loadConfigBaseApp('servers', app.getBase() + '/config/servers.json');  
...
var loadMaster = function(app) {
  app.loadConfigBaseApp('master', app.getBase() + '/config/master.json');
...
var configLogger = function(app) {
  if (process.env.POMELO_LOGGER !== 'off') {
    var env = app.get('env');
    var originPath = path.join(app.getBase(), '/config/log4js.json');
    var presentPath = path.join(app.getBase(), '/config', env, 'log4js.json');
