...
var loadServers = function(app) {
  ...
  for(var serverType in servers) {
    ...
    for(i=0, l=slist.length; i<l; i++) {
      if(slist[i][Constants.Reserved.START_CLUSTER]) {
        loadCluster(serverType, slist[i], serverMap);
        continue;
      }
      server = slist[i];
 ...
 *
 */
var loadCluster = function(serverType, server, serverMap) {
  server.serverType = serverType;
  var portRange = server.port;
  if(portRange.indexOf('-') < 0) {
    logger.warn('cluster server port configuration is error: %j', server);
    return;
  }
  var minPort = parseInt(portRange.split('-')[0]);
  var maxPort = parseInt(portRange.split('-')[1]);
  var count = parseInt(server[Constants.Reserved.CLUSTER_COUNT]);
  var host = server.host;
  if((maxPort-minPort) + 1 < count) {
    logger.warn('cluster server port scope is error: %j', server);
    return;
  }
  ...
  for(var i=0; i<count; i++) {
    ...
    cserver.id = Constants.Reserved.CLUSTER_PREFIX + serverType + '-' + i;
    cserver.port = minPort + i;
