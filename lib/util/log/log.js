...
var getLogger = function(filename) {
	if (!serverId){
		throw new Error('pomelo env not inited! should invoke pomelo.init() in the first place');
	}
	if(!filename) {
		 return log4js.getLogger(serverId);
	}
	return log4js.getLogger(serverId + '  ' + filename.replace(process.cwd(),''));
};
