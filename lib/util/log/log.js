...
var configure = function(logConfFile){
  logConf = require(logConfFile);
	...
	for (var i = 0; i < appenders.length; i++) {
		if (appenders[i].type === 'console'){
			log4js.addAppender(log4js.consoleAppender());
		}
		else if (appenders[i].type === 'file'){
			if (!appenders[i].serverType || appenders[i].serverType === serverType){  // certain serverType need log
				var filename = appenders[i].filename;
				filename = filename.replace('[serverId]', serverId);
				log4js.addAppender(log4js.fileAppender(filename), appenders[i].category);
			}
		}
	};
}
