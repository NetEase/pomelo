...
exp.getSysRemotePath = function(role) {
  var p = path.join(__dirname, Constants.Dir.REMOTE, role);
...
exp.getUserRemotePath = function(appBase, serverType) {
  var p = path.join(appBase, Constants.Dir.SERVER, serverType, 'remote');
  return fs.existsSync(p) ? p : null;
};
...
exp.getCronPath = function(appBase, serverType) {
  var p = path.join(appBase, Constants.Dir.SERVER, serverType, 'cron');
  return fs.existsSync(p) ? p : null;
};
...
exp.listUserRemoteDir = function(appBase) {
  var base = path.join(appBase, Constants.Dir.SERVER);
...
exp.getHandlerPath = function(appBase, serverType) {
  var p = path.join(appBase, Constants.Dir.SERVER, serverType, 'handler');
  return fs.existsSync(p) ? p : null;
};
