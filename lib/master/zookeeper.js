...
Zookeeper.prototype.connect = function(cb){
...
function checkLock(zookeeper, cb){
  var client = zookeeper.client;
  var version = zookeeper.version;
  var lockPath = zookeeper.lockPath;
  ...
  client.getChildren(lockPath, function(err, children, stat){
    ...
    if(version === versions[0]){
      ...
      if(node){
        watchNode(client, nodePath, onNodeChange.bind(zookeeper));
        cb(null, false);
        return;
      }else{
