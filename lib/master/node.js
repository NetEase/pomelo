...
var Node = function(nodeType,nodeId, logs, socket, server) {
  ...
  switch(this.nodeType){
    case 'area':socket.join('areas');break;
    case 'connector':socket.join('connectors');break;
    case 'logic':socket.join('logics');break;
    case 'login':socket.join('logins');break;
    case 'status':socket.join('statuss');
  }
