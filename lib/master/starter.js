 ...
 */
 starter.runServers = function(app) {
  ...
  var unStartServers = app.get('unstarted');
  for(var key in servers) {
    if(!unStartServers || unStartServers.indexOf(key) < 0) {
      var servers_arr = servers[key];
      for(var i=0; i<servers_arr.length; i++) {
        this.run(app, servers_arr[i]);
      }
    }
  }
};
...
starter.sshrun = function(cmd, host, cb) {
  ...
  var app = pomelo.app;
  var args = [];
  var ssh_config = app.get(Constants.Reserved.SSH_CONFIG_PARAMS);
  if(!!ssh_config) {
    args = args.concat(ssh_config);
  }
  args.push(host);
  args.push(cmd);
