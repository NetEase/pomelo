...
var spawnProcess = function(command, host, options, cb) {
  ...
  //auto recover local child process when it error exit
  if(command !== 'ssh'){
    child.on('error', function (err) {
        logger.warn("child process error: error = " + err);
    });
    child.on('exit', function (code, signal) {
        logger.warn("child process exit: code = " + code + " signal = " + signal);
        if(code !== 0){
            logger.warn("try to restart this child process,because last exit code !== 0");
			logger.warn("spawnProcess : command = " + command);
			logger.warn("spawnProcess : host = " + host);
			logger.warn("spawnProcess : options = " + options);
			spawnProcess(command, host, options, cb);
        }
    });
    child.on('close', function (code, signal) {
        logger.warn("child process close: code = " + code + " signal = " + signal);
    });
    child.on('disconnect', function () {
        logger.warn("child process disconnect");
    });
  }
