...
starter.run = function(app, server, cb) {
  var cmd, key, env, daemonize;
  env = app.get(Constants.RESERVED.ENV);
  daemonize = app.get(Constants.RESERVED.DAEMONIZE);
  if (utils.isLocal(server.host)) {
    ...
    options.push(util.format('daemonize=%s', daemonize));
    for(key in server) {
    ...
    cmd += util.format(' "%s" env=%s  daemonize=%s ', app.get(Constants.RESERVED.MAIN), env, daemonize);
    for(key in server) {
      if(key === Constants.RESERVED.CPU) {
        cpus[server.id] = server[key];
      }
