...
LogNode.prototype.render = function() {
  var log_file = this;
  ...
  dom.find(".screens .stream").click(function() {
    if ($(this).is(":checked")) {
      try {
      	$('#stream_template').find('.console').html('');
      	$('#stream_template').find('.console').html('');
        ...
        log_file.node.web_client.socket.on('webmessage',function(message){
	        	if (message.method==='getSystem'){
	        		$('#history_template2').find('.console').html(formatMsg(message.body));
	        	} else {
	        		$('#history_template').find('.console').html(formatMsg(message.body));
	        	};
        	}
        );
        log_file.node.web_client.socket.emit('webmessage',{method:'getSystem',id:serverId});
        log_file.node.web_client.socket.emit('webmessage',{method:'getApp',id:serverId});
      } catch(err) {
        $(this).attr("checked","");
        alert(err);
      }
...
LogNode.prototype.ping = function() {
  var log_file = this;
