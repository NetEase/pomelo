...
var Node = function(label,lognodes,logfiles, web_client) {
  this._dom;
  this.label = label;
  this.log_nodes = {};
  this.log_files = {};
  ...
  var node = this;
   // Create new LogFiles
  _(lognodes).each(function(llabel, num) {
    var log_node = new _ln.LogNode(node, llabel);
    node.log_nodes[llabel] = log_node;
    node.id = llabel;
  });
  _(logfiles).each(function(llabel, num) {
	 var log_file = new _lf.LogFile(node, llabel);
	 node.log_files[node.id] = log_file;
  });
}
