...
var WebClient = function(io) {
  ...
  this.socket = io.connect('http://'+window.location.hostname+':3005');
  ...
  this.socket.on('add_node', function(message) {
  	for (var id in message.logs) {
  		if (!wc.ids[message.logs[id]]){
  			wc.add_node(message.node, message.logs);
  			wc.ids[message.logs[id]] = message.logs[id];
  		} else {
  			console.log('duplicated server add ' + message.logs[id]);
  		}
  	}
