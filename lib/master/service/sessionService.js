var utils = require('../../util/utils');

var exp = module.exports;

/**
 * status service
 * keep all connection infos in memory.
 * only one connection at the same time for each account. latest login will kick the old one off.
 */
var Service = function(opts) {
	// key: user id, value: server id
	this.uidMap = {};
};

var pro = Service.prototype;

/**
 * add uid and serverId pair into status records
 * kick the old login if any.
 * 
 * @param uid user id
 * @param sid server id
 * @param cb(err)
 */
pro.addStatus = function(uid, sid, cb) {
	var oldSid = this.uidMap[uid];

	var self = this;
	function addNewStatus(err) {
		self.uidMap[uid] = sid;
		utils.invokeCallback(cb, err);
	}

	if(!!oldSid) {
		kick(uid, oldSid, addNewStatus);
	} else {
		addNewStatus();
	}

	
};

/**
 * remove status record by uid
 *
 * @param uid user id
 * @param cb(err)
 */
pro.removeStatus = function(uid, cb) {
	delete this.uidMap[uid];

	utils.invokeCallback(cb);
};

/**
 * query status by uid
 * 
 * @param {String} uid user id
 * @param {Function} cb(err, sid) sid: server id corresponsed with the uid or null/undefined for none
 */
pro.queryStatus = function(uid, cb) {
	utils.invokeCallback(cb, null, this.uidMap[uid]);
};

/**
 * query status by uids
 *
 * @param {Array} uids list of uid
 * @param {Function} cb(err, result, missing) result:{sid:[uid]}, missing:[missingUid]
 */
pro.queryStatusBatch = function(uids, cb) {
	var result = {};
	var missing = [];

	var sid, list;
	for(var i=0, l=uids.length; i<l; i++) {
		sid = this.uidMap[uids[i]];
		if(!sid) {
			missing.push(uids[i]);
			continue;
		}

		list = result[sid];
		if(!list) {
			list = [];
			result[sid] = list;
		}
		list.push(uids[i]);
	}

	utils.invokeCallback(cb, null, result, missing);
};

var kick = function(uid, sid, cb) {

};
