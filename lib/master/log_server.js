...
LogServer.prototype = {
  ...
  register: function() {
    ...
    //Brocast nodeId,nodeType,hostName,Mem,loadAvg,CPU,state***********************************************************************
    setInterval(function(){
       var msg=[];
       var nodes=log_server.nodes;
       var sysMsg=monitor.getSysInfo();
       for(var nodeId in nodes){
         msg.push({'nodeId':nodeId,'nodeType':nodes[nodeId].nodeType,'hostName':sysMsg.hostname,'freeMen/totalMem':sysMsg.freemem/sysMsg.totalmem,'loadAvg':sysMsg.loadavg,'cpu':JSON.stringify((sysMsg.cpus)[0].times)});
       }
       log_server.io.sockets.in('web_clients').emit('gkGrid',{'nodes':msg});
    },1000);
    
//    setInterval(function(){
//      var msg=monitorService.getDataMap();
//      log_server.io.sockets.in('web_clients').emit('history_response',msg);
