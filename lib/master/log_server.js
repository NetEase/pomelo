...
//    },1000);
    ...
    socket.on('monitorLog',function(msg){
        // console.error('log_server '+msg);

        log_server.io.sockets.in('web_clients').emit('logMessage', msg);
    });
  ...
  register: function() {
    ...
    log_server.io.sockets.on('connection', function(socket) {
      ...
      socket.on('announce_web_client', function(message) {
        ...
        //log message
        socket.on('logMessage',function(){
          log_server.io.sockets.in('nodes').emit('monitorLog');
          setInterval(function(){
            log_server.io.sockets.in('nodes').emit('monitorLog');
          },60000);
