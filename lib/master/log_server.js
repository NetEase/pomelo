  ...
  announce_node: function(socket, message) {
    ...
    socket.on('monitorSys',function(msg){
      ...
      var wholeMsg={
        ...
        log_server.io.sockets.in('web_clients').emit('systemInfo',oneData);
    });
    socket.on('monitorPro',function(msg){
        node.info.processInfo=msg.body;
        log_server.io.sockets.in('web_clients').emit('processInfo',{nodeItems:msg.body});
    });
    ...
    socket.on('clientmessage', function(msg) {
      if(msg.method=="getSystem"){
        ...
        logger.error('get connection info:%j', msg);
      } else {
   ...
   //monitorLog contains connector and rpc logs
    socket.on('monitorLog',function(msg){
        console.error('log_server monitorLog '+msg.logfile);
    ...
    log_server.io.sockets.on('connection', function(socket) {
      ...
      socket.on('announce_web_client', function(message) {
        ...
        socket.on('con-log',function(msg){
          console.error('log_server con-log msg='+JSON.stringify(msg));
          log_server.io.sockets.in('logics').emit('monitorLog',msg);    
        });
        //rpc log message
         ...
         socket.on('sceneInfo',function(msg){
          console.error('log_server  '+JSON.stringify(msg));

          log_server.io.sockets.in('nodes').emit('monitorScene',msg);
          // setInterval(function(){
          //   log_server.io.sockets.in('nodes').emit('monitorScene',msg);
          // },60000);
