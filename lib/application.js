...
var pinfo = require('pomelo-logger').getLogger('pinfo', __filename);
var perror = require('pomelo-logger').getLogger('perror', __filename);
var pwarn = require('pomelo-logger').getLogger('pwarn', __filename);
...
Application.init = function(opts) {
  ...
  pinfo.info('application inited: %j', this.getServerId());
};
...
Application.load = function(name, component, opts) {
  ...
  if(name && this.components[name]) {
    ...
    pwarn.warn('ignore duplicate component: %j', name);
    return;
  }
...
Application.start = function(cb) {
  ...
  appUtil.optComponents(this.loaded, 'start', function(err) {
    ...
    if(err) {
      ...
      pinfo.info('%j enter after start...', self.getServerId());
      self.afterStart(cb);
    }
  });
};
...
Application.afterStart = function(cb) {
  ...
  appUtil.optComponents(this.loaded, 'afterStart', function(err) {
    ...
    if(!err) {
      perror.error('%j finish start', self.getServerId());
    }
    utils.invokeCallback(cb, err);
  });
};
...
Application.stop = function(force) {
  if(this.state > STATE_STARTED) {
    pwarn.warn('[pomelo application] application is not running now.');
    return;
  }
