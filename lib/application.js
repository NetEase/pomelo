...
app.configAreas = function(val){
  var result = require(app.get('dirname')+'/config/areas.json').areas;
  var servers = app.get('areas');
  var map = {};
...
app.enable = function(setting){
  ...
  //console.log('enable');
  path.exists(settingPath , function(exists){
			console.log(' !!!!!!!enable'+ exists+' path '+ settingPath);
      if (exists) {
...
var loadSysRemote = function(server) {
	...
	logger.error('proxy: %j', app.get('proxyMap').sys);
};
