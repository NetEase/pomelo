		...
		return Proxy.createProxy({
			...
			callback: function(namespace, method, args, attach, invoke) {
				...
				app.get('mailRouter').route({type: attach.type, uid: uid}, function(err, serverId) {
					...
					console.log('app.get(mailBox)=========' + app.get('mailBox'));
...
var loadSysRemote = function(server) {
	app.genRemote(server.serverType, __dirname + '/common/remote', 'sys');
	app.genRemote(server.serverType, __dirname + '/' + server.serverType + '/remote', 'sys');
};

var loadSysProxy = function() {
	app.genProxy('connector', __dirname + '/common/remote', 'sys');
	app.genProxy('connector', __dirname + '/connector/remote', 'sys');
	app.genProxy('logic', __dirname + '/common/remote', 'sys');
	app.genProxy('logic', __dirname + '/logic/remote', 'sys');
	app.genProxy('area', __dirname + '/common/remote', 'sys');
	app.genProxy('area', __dirname + '/area/remote', 'sys');
};

app.run = function(server){
	var cmd = 'cd '+ process.cwd()+ ' && node '+app.main+' production ' + server.serverType + ' ' + server.id;
	if (server.host =='127.0.0.1' || server.host == 'localhost') {
		cmd = 'node ' + process.cwd() +app.main.substring(process.cwd().length,app.main.length)+' production ' + server.serverType + ' ' + server.id;
		//cmd = 'node ' + process.cwd() +'/demos/webrunner/app.js production ' + server.serverType + ' ' + server.id;
		starter.run(cmd);
	} else {
...
app.startMonitor = function() {
  ...
  //console.log('%j',server);
  var monitorClient = require('./monitor/monitorClient.js');
  //or (var i=0; i<typeServers.length; i++){
    //var server = typeServers[i];	
    monitorClient.start(this,server);
  //}
};
