...
app.genHandler = function(name, dir) {
  ...
  path.exists(dir, function(exists){
    ...
    try {
        handlerMap[name] = Loader.loadPath({path: dir, log:true, recurse: false, rootObj: services});
    } catch(err) {
...
app.genProxy = function(name, dir, scope) {
  ...
  path.exists(dir, function(exists){
    ...
    try {
      sobj[name] = Loader.loadPath({path: dir, recurse: false, log:true, namespace: scope + '.' + name, rootObj: services, callback: proxyCallback});
    } catch(err) {
