...
Application.loadConfigBaseApp = function (key, val) {
  ...
  var presentPath = path.join(Application.getBase(), '/config', env, path.basename(val));
...
Application.afterStart = function(cb) {
  ...
  appUtil.optComponents(this.loaded, 'afterStart', function(err) {
    ...
    utils.invokeCallback(cb, err);
    self.event.emit('start_server', self.serverId);
  });
};
...
Application.replaceServers = function(servers) {
  ...
  logger.error('%s this serverArray: %j', this.serverId, serverArray.length);
  logger.error('%s this serversTypesMap: %j', this.serverId, utils.size(this.serverTypes));
  logger.error('%s this servers: %j', this.serverId, utils.size(this.servers));
};
