...
app.enable = function(setting){
  var settingPath = './common/service/' + setting + '.js';
...
app.genHandler = function(name, dir) {
  ...
  try {
      handlerMap[name] = Loader.loadPath({path: dir, log:false, recurse: false, rootObj: services});
  } catch(err) {
...
app.genRemote = function(name, dir, scope) {
  ...
  try {
    ...
    logger.error('[genRemoteError] name:' + name + ', err message:' + err.message);
    process.exit(1);
  }
};
...
app.genProxy = function(name, dir, scope) {
  ...
  try {
    sobj[name] = Loader.loadPath({path: dir, recurse: false, log:false, namespace: scope + '.' + name, rootObj: services, callback: proxyCallback});
  } catch(err) {
    logger.error('[genProxyError] name:' + name + ', err message:' + err.message);
    process.exit(1);
  }
}; 	//end of genProxy
...
app.findServer = function(serverType, serverId){
    var servers = app.get('servers')[app.get('env')];//[app.env];
