...
app.genHandler = function(name, dir) {
	...
	try {
		...
		logger.error('fail to load handler for name:' + name +'  dir:'+dir+ ', err message:' + err.message);
		process.exit(1);
	}
};
...
app.genRemote = function(name, dir, scope) {
	...
	try {
		...
		logger.error('fail to load remote service for ' + name + ', err message:' + err.message);
		process.exit(1);
	}
};
...
app.genProxy = function(name, dir, scope) {
	...
	try {
		...
		logger.error('fail to load proxy for ' + name + ', err message:' + err.message);
		process.exit(1);
	}
}; 	//end of genProxy
