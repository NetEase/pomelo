 ...
 *  };
     ...
     * @memberOf Application
     */
	static set(setting, val, attach, ...args)
    {
		if (args.length === 1)
		{
 ...
 *  });
	...
	static configure(env, type, fn)
    {
		...
		if (env === Constants.RESERVED.ALL || contains(Application.settings.env, env))
		{
			if (type === Constants.RESERVED.ALL || contains(Application.settings.serverType, type))
			{
				fn(this);
			}
		}
