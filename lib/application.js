...
Application.loadServers = function() {
  ...
  var serverMap = {}, serverTypes = [], slist, i, l, server;
  for(var serverType in servers) {
    if(serverTypes.indexOf(serverType) < 0) {
  ...
  this.set('__serverTypes__', serverTypes);
};
...
Application.start = function(cb) {
  ...
  this._optComponents('start', function(err) {
    ...
    if(err) {
      ...
      logger.info('%j enter after start...', self.get('serverId'));
      self.afterStart(cb);
    }
  });
};
...
Application.afterStart = function(cb) {
  ...
  this._optComponents('afterStart', function(err) {
    ...
    if(!err) {
      logger.info('%j finish start', self.get('serverId'));
    }
    utils.invokeCallback(cb, err);
  });
};
...
Application.getServerTypes = function() {
  return this.get('__serverTypes__');
};
...
Application.addServer = function(server) {
  if(!server || !server.id) {
    return;
  }

  var old = this.servers[server.id];
  if(old == server) {
    return;
  }

  this.servers[server.id] = server;
  this.event.emit(events.SERVER_ADD, server);
};

Application.removeServer = function(id) {
  delete this.servers[id];
  this.event.emit(events.SERVER_REMOVE, id);
};
