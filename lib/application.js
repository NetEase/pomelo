...
Application.globalBefore = function (bf) {
  var befores = this.get('__globalBefores__');
  if(!befores) {
    befores = [];
    this.set('__globalBefores__', befores);
  }
  befores.push(bf);
  return this;
};
...
Application.globalAfter = function (af) {
  var afters = this.get('__globalAfters__');
  if(!afters) {
    afters = [];
    this.set('__globalAfters__', afters);
  }
  afters.push(af);
  return this;
};
...
Application.use = function(plugin, opts) {
  ...
  if(!plugin.events) {
    ...
    fs.readdirSync(plugin.events).forEach(function (filename) {
      ...
      var absolutePath = path.join(dir, 'events', filename);
      if(!fs.existsSync(absolutePath)) {
...
Application.transaction = function(name, conditions, handlers, retry) {
  ...
  if(typeof name !== 'string') {
    logger.error('transaction name is error format.');
    return;
  }
  if(typeof conditions !== 'object' || typeof handlers !== 'object') {
    logger.error('transaction parameters is error format.');
    return;
  }
  ...
  for(var key in conditions) {
    if(typeof key !== 'string' || typeof conditions[key] !== 'function') {
      logger.error('transaction conditions parameters is error format.');
      return;
    }
    cnames.push(key);
      ...
      process.nextTick(function() {
        for(var key in handlers) {
          if(typeof key !== 'string' || typeof handlers[key] !== 'function') {
            logger.error('transcation handlers parameters is error format.');
            return;
          }
          dnames.push(key);
        ...
        async.whilst(
          ...
          function(callback) {
            ...
            async.forEachSeries(dmethods, function(method, cb) { 
              ...
              callback();
              process.nextTick(function() {
                transactionLogger.info('[%s] all conditions and handlers execute successfully.', name);
              });
            });
          },
          function(err) {
...
/**
 * Add activities from current application at runtime.
 *
 * @param  {Array} activities new activities would be added in application
 * @memberOf Application
 */
Application.addActivities = function(activities) {
  if(!activities || !activities.length) {
    logger.warn('activities is not defined.');
    return;
  }
  this.event.emit(events.ADD_ACTIVITIES, activities);
};

/**
 * Remove activities from current application at runtime.
 *
 * @param  {Array} activities old activities would be removed in application
 * @memberOf Application
 */
Application.removeActivities = function(activities) {
  if(!activities || !activities.length) {
    logger.warn('ids is not defined.');
    return;
  }
  this.event.emit(events.REMOVE_ACTIVITIES, activities);
};
